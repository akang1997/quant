...
export interface IStrategy {
     ...
     * crtTime : 当前时间字符串
     */
    tick(account: Account, order: Order, crtTime: string) :void
    // TODO 每个月跑一次，可以设置是每个月的哪一天；收盘后执行
    run_monthly(account: Account, order: Order, crtTime: string):void
    // TODO 每个周跑一次，可以设置是每个周的哪一天；收盘后执行
    run_weekly(account: Account, order: Order, crtTime: string):void
    // TODO 每天跑一次，；收盘后执行 ？？
    run_daily(account: Account, order: Order, crtTime: string):void
}
